...
<body>
  <div id="wrapper">
    <header>
      ...
      <div id="header-user">
        <?php if (empty($_SESSION['login']) or empty($_SESSION['id']) or empty($_SESSION['id1C']))     
        {     
    ...
    <aside>
      <nav>
        <ul class="aside-menu">
          ...
          <?php 
          if ($_SESSION['isGroup']=="2"){
           echo "<li><a href='users/'>СПИСОК ПОЛЬЗОВАТЕЛЕЙ</a></li><li><a href='blocker/'>БЛОКИРОВКА</a></li>";
         }
         ?>
         <?php 
         if ($_SESSION['isGroup']=="0") {
          echo "<li><a href='work/'>ЗАДОЛЖЕННОСТИ</a></li>
          <li><a href='request/'>ОТГРУЗКИ</a></li>
          <li><a href='marshrut/'>МАРШРУТ</a></li>
          <li><a href='kontagent/'>КОНТРАГЕНТЫ</a></li>
          <li><a href='penni/'>ПЕННИ</a></li>
          <li><a href='zadania/'>ЗАДАНИЯ</a></li>
		  <li><a href='aktsverki/'>АКТЫ СВЕРКИ</a></li>";
		  
        }
        ?>
        <?php 
        if ($_SESSION['isGroup']=="3"){
         echo "<li><a href='riders/'>ПУТЕВЫЕ ЛИСТЫ</a></li>";
       }
       ?>
     </ul>
   </nav>
...
// 
  ...
  $result = mysql_query("SELECT * FROM users WHERE id1C=".$_SESSION['id1C'],$db);     
     ...
     { 
	  ...
	  Номер сессии : ".$_SESSION['id']." <br> "."
	  Уровень доступа : ".$_SESSION['isGroup']." <br> "."
	  ip адрес : ".$_SESSION['ip']." <br> "."
	  Браузер : ".$_SESSION['browser']." <br> "."
	  Телефон : ".$myrow['telephone']." <br> "."
	  Email : ".$myrow['email']." <br> "."
	  Код из 1С : ".$myrow['id1C']." <br> "."
	  Сегодня : ".date("d.m.y")." <br> "); 
